...
void iot_i2s_lcd_write_data(i2s_lcd_handle_t i2s_lcd_handle, uint16_t data)
{
    ...
    if(i2s_lcd->i2s_lcd_conf.cs_io_num) GPIO.out_w1tc = (1 << i2s_lcd->i2s_lcd_conf.cs_io_num);
    i2s_lcd_write_data(i2s_lcd_handle, (char *)&data, 2, 100, true);
    if(i2s_lcd->i2s_lcd_conf.cs_io_num) GPIO.out_w1ts = (1 << i2s_lcd->i2s_lcd_conf.cs_io_num);
}
...
void iot_i2s_lcd_write_data(i2s_lcd_handle_t i2s_lcd_handle, uint16_t data)
{
    i2s_lcd_t *i2s_lcd = (i2s_lcd_t *)i2s_lcd_handle;
    i2s_port_t i2s_num = i2s_lcd->i2s_port;
    if(i2s_lcd_pin_conf->cs_io_num) GPIO.out_w1tc = (1 << i2s_lcd->i2s_lcd_conf.cs_io_num);
    ...
    I2S[i2s_num]->conf.tx_fifo_reset = 1;
    I2S[i2s_num]->conf.tx_fifo_reset = 0;
    if(i2s_lcd_pin_conf->cs_io_num) GPIO.out_w1ts = (1 << i2s_lcd->i2s_lcd_conf.cs_io_num);
}
...
i2s_lcd_handle_t iot_i2s_lcd_pin_cfg(i2s_port_t i2s_port, i2s_lcd_config_t *i2s_lcd_pin_conf)
{
    ...
    gpio_set_direction(i2s_lcd_pin_conf->rst_io_num, GPIO_MODE_OUTPUT);
    gpio_set_pull_mode(i2s_lcd_pin_conf->rst_io_num, GPIO_PULLUP_ONLY);
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[i2s_lcd_pin_conf->rst_io_num], PIN_FUNC_GPIO);

    if(i2s_lcd_pin_conf->cs_io_num) {
        gpio_set_direction(i2s_lcd_pin_conf->cs_io_num, GPIO_MODE_OUTPUT);
        gpio_set_pull_mode(i2s_lcd_pin_conf->cs_io_num, GPIO_PULLUP_ONLY);
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[i2s_lcd_pin_conf->cs_io_num], PIN_FUNC_GPIO);
    }
